...
Daemon::Daemon(int port)
{
	...
	wndclass.hInstance		= GetVNCApp()->m_instance;
	...
	// Load a popup menu
	m_hmenu = LoadMenu(GetVNCApp()->m_instance, MAKEINTRESOURCE(IDR_TRAYMENU));
...
void Daemon::AddTrayIcon() {
	GetVNCLog().Print(4, _T("Adding tray icon\n"));
	SendTrayMsg(NIM_ADD);
}

void Daemon::CheckTrayIcon() {
	GetVNCLog().Print(8, _T("Checking tray icon\n"));
	if (!SendTrayMsg(NIM_MODIFY)) {
		GetVNCLog().Print(4, _T("Tray icon not there - reinstalling\n"));
		AddTrayIcon();
	};
}

void Daemon::RemoveTrayIcon() {
	GetVNCLog().Print(4, _T("Deleting tray icon\n"));
...
bool Daemon::SendTrayMsg(DWORD msg)
{
	...
	m_nid.hIcon = LoadIcon(GetVNCApp()->m_instance, MAKEINTRESOURCE(IDR_TRAY));
	m_nid.uFlags = NIF_ICON | NIF_MESSAGE;
	m_nid.uCallbackMessage = WM_TRAYNOTIFY;
	m_nid.szTip[0] = '\0';
	// Use resource string as tip if there is one
	if (LoadString(GetVNCApp()->m_instance, IDR_TRAY, m_nid.szTip, sizeof(m_nid.szTip))) {
...
LRESULT CALLBACK Daemon::WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_SOCKEVENT:
		{
			...
			case FD_ACCEPT:
				{
					...
					GetVNCApp()->NewConnection(hNewSock);
			...
			case FD_CLOSE:
				GetVNCLog().Print(5, _T("Daemon connection closed\n"));
				DestroyWindow(hwnd);
				break;
			}
	...
	case WM_COMMAND:
		...
		case ID_NEWCONN:
			GetVNCApp()->NewConnection();
			break;
		case IDC_OPTIONBUTTON:
			GetVNCApp()->m_options.DoDialog();
			GetVNCApp()->m_options.SaveOpt(".listen", KEY_VNCVIEWER_HISTORI);
			break;
		case ID_CLOSEDAEMON:
			PostQuitMessage(0);
			break;
	...
	case WM_TRAYNOTIFY:
		{
			...
			if (lParam==WM_LBUTTONDBLCLK) {
				...
				if (hSubMenu == NULL) { 
					GetVNCLog().Print(2, _T("No systray submenu\n"));
					return 0;
				}
