...
{
	GetVNCLog().Print(2, _T("Clipboard changed\n"));
	
	HWND hOwner = GetClipboardOwner();
	if (hOwner == m_hwnd) {
		GetVNCLog().Print(2, _T("We changed it - ignore!\n"));
	} else if (!m_initialClipboardSeen) {
		GetVNCLog().Print(2, _T("Don't send initial clipboard!\n"));
		...
		if (OpenClipboard(m_hwnd)) { 
			...
			if (hglb == NULL) {
				...
				try {
					...
					GetVNCLog().Print(0, _T("Exception while sending clipboard text : %s\n"), e.m_info);
