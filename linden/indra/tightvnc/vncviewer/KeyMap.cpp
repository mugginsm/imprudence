...
KeyActionSpec KeyMap::PCtoX(UINT virtkey, DWORD keyData) { 
    ...
    GetVNCLog().Print(8,_T(" keyData %04x "), keyData);
    
    if (extended) { 
        GetVNCLog().Print(8, _T(" (extended) "));
        switch (virtkey) {
        case VK_MENU :
    ...
    if (numkeys != 0) {
        ...
        GetVNCLog().Print(8, _T("keymap gives %u (%x) "), key, key);

    } else {
        // not found in table
        GetVNCLog().Print(8, _T("not in special keymap, "));
        ...
        if (  ((keystate[VK_MENU] & 0x80) != 0) &&
           ...
           if  ( (ret >= 1) && 
               ...
               GetVNCLog().Print(8, _T("Ctrl-Alt pressed: ToAscii (without modifiers) returns %d byte(s): "), ret);
                for (int i = 0; i < ret; i++) {
                   kas.keycodes[numkeys++] = *(buf+i);
                   GetVNCLog().Print(8, _T("%02x (%c) "), *(buf+i) , *(buf+i));
                }
                GetVNCLog().Print(8,_T("\n"));
           } 
        ...
        if (numkeys == 0) {
            ...
            if (ret >= 1) {
                GetVNCLog().Print(8,_T("ToAscii (without ctrl) returns %d byte(s): "), ret);
                for (int i = 0; i < ret; i++) {
                   kas.keycodes[numkeys++] = *(buf+i);
                  GetVNCLog().Print(8, _T("%02x (%c) "), *(buf+i) , *(buf+i));
                }
