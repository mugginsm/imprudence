...
void VNCOptions::Register()
{
	...
	if ( RegCreateKey(HKEY_CLASSES_ROOT, ".vnc", &hKey)  == ERROR_SUCCESS ) {
		...
		GetVNCLog().Print(0, "Failed to register .vnc extension\n");
	}

	char filename[_MAX_PATH];
	if (GetModuleFileName(NULL, filename, _MAX_PATH) == 0) {
		GetVNCLog().Print(0, "Error getting vncviewer filename\n");
		return;
	}
	GetVNCLog().Print(2, "Viewer is %s\n", filename);
...
{
	m_running = running;
	return DialogBoxParam(GetVNCApp()->m_instance, DIALOG_MAKEINTRESOURCE(IDD_PARENT), 
	...
	case WM_INITDIALOG: 
		{
			...
			_this->m_hPageConnection = CreateDialogParam(GetVNCApp()->m_instance,
				MAKEINTRESOURCE(IDD_OPTIONDIALOG),
				hwndDlg,
				(DLGPROC)_this->DlgProcConnOptions,
				(LONG)_this);

			_this->m_hPageGeneral = CreateDialogParam(GetVNCApp()->m_instance, 
											   ...
											   WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG: 
		{					
			...
			SetDlgItemInt( hwnd, IDC_LISTEN_PORT, GetVNCApp()->m_options.m_listenPort, FALSE);
			
			HWND hSpin3 = GetDlgItem(hwnd, IDC_SPIN3);
			SendMessage(hSpin3, UDM_SETBUDDY, (WPARAM) (HWND)hListenPort, 0);
			
			SendMessage(hChec, BM_SETCHECK, GetVNCApp()->m_options.m_logToFile, 0);
			_this->EnableLog(hwnd, !(SendMessage(hChec, BM_GETCHECK, 0, 0) == 0));				
			SetDlgItemInt( hwnd, IDC_EDIT_LOG_LEVEL, GetVNCApp()->m_options.m_logLevel, FALSE);
			SetDlgItemText( hwnd, IDC_EDIT_LOG_FILE, GetVNCApp()->m_options.m_logFilename);
			return TRUE;
		}
	case WM_HELP:
		GetVNCHelp().Popup(lParam);
		...
		case IDC_BUTTON_CLEAR_LIST: 
			{
				...
				while (RegEnumValue(hRegKey, index, value, &valuesize,
					...
					GetVNCApp()->m_options.delkey(data, KEY_VNCVIEWER_HISTORI);
		...
		case IDC_OK:
			{			
				...
				if (SendMessage(hDotCursor, BM_GETCHECK, 0, 0) == BST_CHECKED) {
					GetVNCApp()->m_options.m_localCursor = DOTCURSOR;
					SetClassLong(_this->m_hWindow, GCL_HCURSOR,
									(long)LoadCursor(GetVNCApp()->m_instance, 
									MAKEINTRESOURCE(IDC_DOTCURSOR)));
				}
				if (SendMessage(hSmallCursor, BM_GETCHECK, 0, 0) == BST_CHECKED) {
					GetVNCApp()->m_options.m_localCursor = SMALLCURSOR;
					SetClassLong(_this->m_hWindow, GCL_HCURSOR,
									(long)LoadCursor(GetVNCApp()->m_instance, 
									MAKEINTRESOURCE(IDC_SMALLDOT)));
				}
				if (SendMessage(hNoCursor, BM_GETCHECK, 0, 0) == BST_CHECKED) {
					GetVNCApp()->m_options.m_localCursor = NOCURSOR;
					SetClassLong(_this->m_hWindow, GCL_HCURSOR,
									(long)LoadCursor(GetVNCApp()->m_instance, 
									MAKEINTRESOURCE(IDC_NOCURSOR)));
				}
				if (SendMessage(hNormalCursor, BM_GETCHECK, 0, 0) == BST_CHECKED) {
					GetVNCApp()->m_options.m_localCursor = NORMALCURSOR;
					SetClassLong(_this->m_hWindow, GCL_HCURSOR,
									(long)LoadCursor(NULL,IDC_ARROW));
				}
				
				if (SendMessage(hChec, BM_GETCHECK, 0, 0) == 0) {
					GetVNCApp()->m_options.m_logToFile = false;
				} else {
					GetVNCApp()->m_options.m_logToFile = true;
					GetVNCApp()->m_options.m_logLevel = GetDlgItemInt(hwnd,
						IDC_EDIT_LOG_LEVEL, NULL, FALSE);
					GetDlgItemText(hwnd, IDC_EDIT_LOG_FILE,
						buf, 80); 
					strcpy(GetVNCApp()->m_options.m_logFilename, buf);
					
					GetVNCLog().SetLevel(GetVNCApp()->m_options.m_logLevel);
					GetVNCLog().SetFile(GetVNCApp()->m_options.m_logFilename);
				}
				
				if (SendMessage(hMessage, BM_GETCHECK, 0, 0) == 0) {
					GetVNCApp()->m_options.m_skipprompt = true;
				} else {
					GetVNCApp()->m_options.m_skipprompt = false;
				}				
				if (SendMessage(hToolbar, BM_GETCHECK, 0, 0) == 0) {
					GetVNCApp()->m_options.m_toolbar = false;					
				} else {
					GetVNCApp()->m_options.m_toolbar = true;					
				}
				
				GetVNCApp()->m_options.m_historyLimit = GetDlgItemInt(hwnd,
					IDC_EDIT_AMOUNT_LIST, NULL, FALSE);
				GetVNCApp()->m_options.m_listenPort = GetDlgItemInt(hwnd,
					IDC_LISTEN_PORT, NULL, FALSE);
				GetVNCApp()->m_options.SaveGenOpt();
...
void VNCOptions::BrowseLogFile()
{
	...
	ofn.lpstrFile = GetVNCApp()->m_options.m_logFilename;
	...
	if (!GetSaveFileName(&ofn)) {
		...
		case FNERR_INVALIDFILENAME:
			...
			break;
		default:
			GetVNCLog().Print(0, "Error %d from GetSaveFileName\n", err);
			break;
		}
		return;
	}
