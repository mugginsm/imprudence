...
void ClientConnection::ReadTightRect(rfbFramebufferUpdateRectHeader *pfburh)
{
  if (m_myFormat.bitsPerPixel != 8 &&
    ...
    GetVNCLog().Print(0, _T("Invalid number of bits per pixel: %d\n"),
    ...
    if ((comp_ctl & 1) && m_tightZlibStreamActive[i]) {
      ...
      if (err != Z_OK) {
        if (m_tightZlibStream[i].msg != NULL) {
          GetVNCLog().Print(0, _T("zlib inflateEnd() error: %s\n"),
                       m_tightZlibStream[i].msg);
        } else {
          GetVNCLog().Print(0, _T("zlib inflateEnd() error: %d\n"), err);
        }
        return;
      }
  ...
  if (comp_ctl >= rfbTightMaxSubencoding) {
    GetVNCLog().Print(0, _T("Tight encoding: bad subencoding value received.\n"));
    return;
  }
  ...
  if ((comp_ctl & rfbTightExplicitFilter) != 0) {
    ...
    case rfbTightFilterGradient:
      ...
      break;
    default:
      GetVNCLog().Print(0, _T("Tight encoding: unknown filter code received.\n"));
      return;
    }
  } else {
    bitsPixel = InitFilterCopy(pfburh->r.w, pfburh->r.h);
  ...
  if (bitsPixel == 0) {
    GetVNCLog().Print(0, _T("Tight encoding: error receiving palette.\n"));
    return;
  }
  ...
  if (compressedLen <= 0) {
    GetVNCLog().Print(0, _T("Tight encoding: bad data received from server.\n"));
    return;
  }
  ...
  if (!m_tightZlibStreamActive[stream_id]) {
    ...
    if (err != Z_OK) {
      if (zs->msg != NULL) {
        GetVNCLog().Print(0, _T("zlib inflateInit() error: %s.\n"), zs->msg);
      } else {
        GetVNCLog().Print(0, _T("zlib inflateInit() error: %d.\n"), err);
      }
      return;
    }
  ...
  while (compressedLen > 0) {
    ...
    do {
      ...
      if (err != Z_OK && err != Z_STREAM_END) {
        if (zs->msg != NULL) {
          GetVNCLog().Print(0, _T("zlib inflate() error: %s.\n"), zs->msg);
        } else {
          GetVNCLog().Print(0, _T("zlib inflate() error: %d.\n"), err);
        }
        return;
      }
  ...
  if (rowsProcessed != pfburh->r.h) {
    GetVNCLog().Print(0, _T("Tight encoding: wrong number of scan lines.\n"));
  }
}
...
void ClientConnection::DecompressJpegRect(int x, int y, int w, int h)
{
  ...
  if (compressedLen <= 0) {
    GetVNCLog().Print(0, _T("Incorrect data received from the server.\n"));
    return;
  }
  ...
  if ((int)cinfo.output_width != w || (int)cinfo.output_height != h ||
    ...
    GetVNCLog().Print(0, _T("Tight Encoding: Wrong JPEG data received.\n"));
