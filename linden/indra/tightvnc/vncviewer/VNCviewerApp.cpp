...
VNCviewerApp::VNCviewerApp(HINSTANCE hInstance, LPTSTR szCmdLine) {
	...
	// Logging info
	GetVNCLog().SetLevel(m_options.m_logLevel);
	if (m_options.m_logToConsole) {
		GetVNCLog().SetMode(Log::ToConsole | Log::ToDebug);
	}
	if (m_options.m_logToFile) {
		GetVNCLog().SetFile(m_options.m_logFilename);
	}

#ifdef PALM_LOG
	// Hack override for WinCE Palm developers who can't give
	// options to commands, not even via shortcuts.
	GetVNCLog().SetLevel(20);
 	GetVNCLog().SetFile(_T("\\Temp\\log"));
#endif
	...
	GetVNCLog().Print(3, _T("Started and Winsock (v %d) initialised\n"), wsaData.wVersion);
}
...
void VNCviewerApp::RegisterConnection(ClientConnection *pConn) {
	...
	for (i = 0; i < MAX_CONNECTIONS; i++) {
		if (m_clilist[i] == NULL) {
			m_clilist[i] = pConn;
			GetVNCLog().Print(4,_T("Registered connection with app\n"));
			return;
		}
	}
	// If we've got here, something is wrong.
	GetVNCLog().Print(-1, _T("Client list overflow!\n"));
...
void VNCviewerApp::DeregisterConnection(ClientConnection *pConn) {
	...
	for (i = 0; i < MAX_CONNECTIONS; i++) {
		if (m_clilist[i] == pConn) {
			...
			GetVNCLog().Print(4,_T("Deregistered connection from app\n"));

			// No clients left? then we should finish, unless we're in
			// listening mode.
			if ((m_clilist[0] == NULL) && (!GetVNCApp()->m_options.m_listening)){
				PostQuitMessage(0);}
	...
	// If we've got here, something is wrong.
	GetVNCLog().Print(-1, _T("Client not found for deregistering!\n"));
	PostQuitMessage(1);
}
...
VNCviewerApp::~VNCviewerApp() {
	...
	GetVNCLog().Print(2, _T("VNC viewer closing down\n"));
