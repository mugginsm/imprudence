	...
	VNCviewerApp(hInstance, szCmdLine)
{
	...
	if ((m_options.m_listening) && (FindWindow("VNCviewer Daemon", 0) == NULL)) {
		GetVNCLog().Print(3, _T("In listening mode - staring daemons\n"));
		ListenMode();
	} else {
...
ClientConnection* VNCviewerApp32::NewConnection() {
	int retries = 0;
...
ClientConnection* VNCviewerApp32::NewConnection(SOCKET sock) {
	int retries = 0;
	ClientConnection *pcc;
	ClientConnection *old_pcc;
