...
BOOL WINAPI DllMain(HINSTANCE hDllInstance, DWORD dwReason, LPVOID pReserved)
{
    if (dwReason == DLL_PROCESS_ATTACH)
        g_hVNCModule  = (void*)hDllInstance;
    else if (dwReason == DLL_PROCESS_DETACH)
        g_hVNCModule = 0;
    return true;
}

TIGHTVNC_EXPORT VNCviewerApp* g_pApp;

TIGHTVNC_EXPORT void* GetVNCModule() { return g_hVNCModule; }
TIGHTVNC_EXPORT void SetVNCApp(VNCviewerApp* app) { g_pApp = app; }
TIGHTVNC_EXPORT VNCviewerApp* GetVNCApp() { return g_pApp; }
TIGHTVNC_EXPORT Log& GetVNCLog() { return g_vnclog; }
TIGHTVNC_EXPORT VNCHelp& GetVNCHelp() { return g_help; }
TIGHTVNC_EXPORT HotKeys& GetVNCHotkeys() { return g_hotkeys; }
...
TIGHTVNC_EXPORT void CentreWindow(HWND hwnd)
{
...
TIGHTVNC_EXPORT bool ParseDisplay(LPTSTR display, LPTSTR phost, int hostlen, int *pport) 
{
...
TIGHTVNC_EXPORT void FormatDisplay(int port, LPTSTR display, LPTSTR host)
{
	if (port == 5900) {
