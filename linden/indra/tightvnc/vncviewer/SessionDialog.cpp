...
{
 	return DialogBoxParam(GetVNCApp()->m_instance, DIALOG_MAKEINTRESOURCE(IDD_SESSION_DLG), 
...
BOOL CALLBACK SessionDialog::SessDlgProc(  HWND hwnd,  UINT uMsg,  WPARAM wParam, LPARAM lParam ) 
{
	...
	case WM_ACTIVATE:
	case WM_ACTIVATEAPP:
		if ((GetVNCApp()->m_options.m_listening) || 
	...
	case WM_COMMAND:		
		...
		case IDC_LIST_MODE: 
			GetVNCApp()->m_options.LoadOpt(".listen", KEY_VNCVIEWER_HISTORI);
			GetVNCApp()->m_options.m_listening=true;
			GetVNCApp()->ListenMode();
