...
void ClientConnection::CreateDisplay() 
{
	...
	if (GetDeviceCaps(m_hBitmapDC, RASTERCAPS) & RC_PALETTE) {
		GetVNCLog().Print(3, _T("Palette-based display - %d entries, %d reserved\n"), 
...
void ClientConnection::SaveListConnection()
{
	if (!m_serverInitiated) {
		...
		int dwbuflen = 255;
		int i, j;
		int maxEntries = GetVNCApp()->m_options.m_historyLimit;
		TCHAR list[80];
		HKEY m_hRegKey;
		TCHAR  buf[256];
		DWORD dispos;
...
void ClientConnection::Connect()
{
	...
	GetVNCLog().Print(0, _T("Connected to %s port %d\n"), m_host, m_port);
...
void ClientConnection::NegotiateProtocolVersion()
{
	...
	GetVNCLog().Print(0, _T("RFB server supports protocol version 3.%d\n"),
				 minorVersion);
	...
	GetVNCLog().Print(0, _T("Connected to RFB server, using protocol version 3.%d\n"),
				 m_minorVersion);
}
...
void ClientConnection::PerformAuthentication()
{
	...
	default:	// should never happen
		GetVNCLog().Print(0, _T("Internal error: Invalid security type\n"));
...
int ClientConnection::ReadSecurityType()
{
	...
	if (secType != rfbSecTypeNone && secType != rfbSecTypeVncAuth) {
		GetVNCLog().Print(0, _T("Unknown security type from RFB server: %d\n"),
					 (int)secType);
...
int ClientConnection::SelectSecurityType()
{
	...
	for (j = 0; j < (int)nSecTypes; j++) {
		if (secTypes[j] == rfbSecTypeTight) {
			...
			GetVNCLog().Print(8, _T("Enabling TightVNC protocol extensions\n"));
	...
	for (j = 0; j < (int)nSecTypes; j++) {
		for (int i = 0; i < nKnownSecTypes; i++) {
			if (secTypes[j] == knownSecTypes[i]) {
				...
				GetVNCLog().Print(8, _T("Choosing security type %s(%d)\n"),
    ...
    if (secType == rfbSecTypeInvalid) {
		GetVNCLog().Print(0, _T("Server did not offer supported security type\n"));
...
void ClientConnection::PerformAuthenticationTight()
{
	...
	if (!caps.nAuthTypes) {
		GetVNCLog().Print(0, _T("No authentication needed\n"));
		if (m_connDlg != NULL)
		...
		if (!m_authCaps.NumEnabled()) {
			GetVNCLog().Print(0, _T("No suitable authentication schemes offered by the server\n"));
...
void ClientConnection::Authenticate(CARD32 authScheme)
{
	...
	***/
	...
	case rfbAuthVNC:
		...
		break;
	default:
		GetVNCLog().Print(0, _T("Unknown authentication scheme: %d\n"),
					 (int)authScheme);
		throw ErrorException("Unknown authentication scheme!");
	}

	GetVNCLog().Print(0, _T("Authentication scheme: %s\n"),
	...
	case rfbAuthOK:
		...
		GetVNCLog().Print(0, _T("Authentication successful\n"));
		return;
	case rfbAuthFailed:
		if (m_minorVersion >= 8) {
...
bool ClientConnection::AuthenticateVNC(char *errBuf, int errBufSize)
{
	...
	if (strlen((const char *) m_encPasswd) > 0) {
		...
		LoginAuthDialog ad(m_host, "Standard VNC Authentication");
		ad.DoDialog();
#ifndef UNDER_CE
...
void ClientConnection::SizeWindow(bool centered)
{
	...
	GetVNCLog().Print(2, _T("Screen work area is %d x %d\n"),
...
void ClientConnection::SetupPixelFormat() {
    ...
    if (m_opts.m_Use8Bit) {		
		...
		GetVNCLog().Print(2, _T("Requesting 8-bit truecolour\n"));  
        ...
        GetVNCLog().Print(2, _T("Requesting 16-bit truecolour\n"));
		...
		if (false) {
			...
			GetVNCLog().Print(2, _T("Memory DC has depth of %d and %s pallete-based.\n"), 
...
{	
	...
	case WM_SYSCHAR:
        ...
        {
            int key = wParam;
            GetVNCLog().Print(4,_T("CHAR msg : %02x\n"), key);
	...
	case WM_KILLFOCUS:
		{
			...
			GetVNCLog().Print(6, _T("Losing focus - cancelling modifiers\n"));
			return 0;
		}	
		...
		if (m_waitingOnEmulateTimer) {
			...
			if ( (msg == WM_LBUTTONDOWN && !(keyflags & MK_RBUTTON))
				...
				if (!m_emulate3ButtonsTimer) {
					GetVNCLog().Print(0, _T("Failed to create timer for emulating 3 buttons"));
...
inline void ClientConnection::ProcessKeyEvent(int virtkey, DWORD keyData)
{
    ...
    if (GetKeyNameText(  keyData,keyname, 31)) {
        GetVNCLog().Print(4, _T("Process key: %s (keyData %04x): "), keyname, keyData);
    };
#endif
#endif
	...
	try {
		...
		if (kas.releaseModifiers & KEYMAP_LCONTROL) {
			SendKeyEvent(XK_Control_L, false );
			GetVNCLog().Print(5, _T("fake L Ctrl raised\n"));
		}
		if (kas.releaseModifiers & KEYMAP_LALT) {
			SendKeyEvent(XK_Alt_L, false );
			GetVNCLog().Print(5, _T("fake L Alt raised\n"));
		}
		if (kas.releaseModifiers & KEYMAP_RCONTROL) {
			SendKeyEvent(XK_Control_R, false );
			GetVNCLog().Print(5, _T("fake R Ctrl raised\n"));
		}
		if (kas.releaseModifiers & KEYMAP_RALT) {
			SendKeyEvent(XK_Alt_R, false );
			GetVNCLog().Print(5, _T("fake R Alt raised\n"));
		}
		
		for (int i = 0; kas.keycodes[i] != XK_VoidSymbol && i < MaxKeysPerKey; i++) {
			SendKeyEvent(kas.keycodes[i], down );
			GetVNCLog().Print(4, _T("Sent keysym %04x (%s)\n"), 
				kas.keycodes[i], down ? _T("press") : _T("release"));
		}
		
		if (kas.releaseModifiers & KEYMAP_RALT) {
			SendKeyEvent(XK_Alt_R, true );
			GetVNCLog().Print(5, _T("fake R Alt pressed\n"));
		}
		if (kas.releaseModifiers & KEYMAP_RCONTROL) {
			SendKeyEvent(XK_Control_R, true );
			GetVNCLog().Print(5, _T("fake R Ctrl pressed\n"));
		}
		if (kas.releaseModifiers & KEYMAP_LALT) {
			SendKeyEvent(XK_Alt_L, false );
			GetVNCLog().Print(5, _T("fake L Alt pressed\n"));
		}
		if (kas.releaseModifiers & KEYMAP_LCONTROL) {
			SendKeyEvent(XK_Control_L, false );
			GetVNCLog().Print(5, _T("fake L Ctrl pressed\n"));
		}
	} catch (Exception &e) {
		e.Report();
		PostMessage(m_hwnd1, WM_CLOSE, 0, 0);
...
void ClientConnection::SendClientCutText(char *str, int len)
{
	...
	WriteExact(str, len);
	GetVNCLog().Print(6, _T("Sent %d bytes of clipboard\n"), len);
}
#endif
...
inline void ClientConnection::DoBlit() 
{
	...
	if (m_opts.m_scaling) {
		...
		if (!StretchBlt(
			...
			SRCCOPY)) 
		{
			GetVNCLog().Print(0, _T("Blit error %d\n"), GetLastError());
...
void* ClientConnection::run_undetached(void* arg) {
	...
	GetVNCLog().Print(9, _T("Update-processing thread started\n"));
		...
		while (!m_bKillThread) {
			...
			CARD8 msgType;
			{
			  ...
			  if (bytes == 0) {
			    ...
			    GetVNCLog().Print(0, _T("Connection closed\n") );
			    throw WarningException(_T("Connection closed"));
			  }
			  if (bytes < 0) {
                m_pFileTransfer->CloseUndoneFileTransfers();
			    GetVNCLog().Print(3, _T("Socket error reading message: %d\n"), WSAGetLastError() );
			...
			default:
				GetVNCLog().Print(3, _T("Unknown message type x%02x\n"), msgType );
				throw WarningException("Unhandled message type received!\n");
			}

		}
        
        GetVNCLog().Print(4, _T("Update-processing thread finishing\n") );
...
ClientConnection::SendFramebufferUpdateRequest(int x, int y, int w, int h, bool incremental)
{
	...
	GetVNCLog().Print(10, _T("Request %s update\n"), incremental ? _T("incremental") : _T("full"));
...
void ClientConnection::SendAppropriateFramebufferUpdateRequest()
{
	if (m_pendingFormatChange) {
		GetVNCLog().Print(3, _T("Requesting new pixel format\n") );
...
void ClientConnection::ReadScreenUpdate() {
	...
	for (int i=0; i < sut.nRects; i++) {
		...
		case rfbEncodingZlibHex:
			...
			break;
		default:
			GetVNCLog().Print(0, _T("Unknown encoding %d - not supported!\n"), surh.encoding);
			break;
		}
...
{
	GetVNCLog().Print(5, _T("%s dormant mode\n"), newstate ? _T("Entering") : _T("Leaving"));
	m_dormant = newstate;
	if (!m_dormant)
		SendIncrementalFramebufferUpdateRequest();
}
...
void ClientConnection::ReadServerCutText() {
	...
	GetVNCLog().Print(6, _T("Read remote clipboard change\n"));
...
void ClientConnection::ReadExact(char *inbuf, int wanted)
{
	...
	int offset = 0;
    GetVNCLog().Print(10, _T("  reading %d bytes\n"), wanted);
	...
	while (wanted > 0) {
		...
		if (bytes == SOCKET_ERROR) {
			...
			GetVNCLog().Print(1, _T("Socket error while reading %d\n"), err);
			m_running = false;
...
inline void ClientConnection::ReadString(char *buf, int length)
{
	...
	buf[length] = '\0';
    GetVNCLog().Print(10, _T("Read a %d-byte string\n"), length);
}
...
inline void ClientConnection::WriteExact(char *buf, int bytes)
{
	...
	GetVNCLog().Print(10, _T("  writing %d bytes\n"), bytes);
    ...
    while (i < bytes) {
		...
		if (j == SOCKET_ERROR || j==0) {
			...
			GetVNCLog().Print(1, _T("Socket error %d: %s\n"), err, lpMsgBuf);
			LocalFree( lpMsgBuf );
			m_running = false;
...
char *ClientConnection::ReadFailureReason()
{
	...
	GetVNCLog().Print(0, _T("RFB connection failed, reason: %s\n"), m_netbuf);
	return m_netbuf;
}
...
void ClientConnection::CheckBufferSize(int bufsize)
{
	...
	GetVNCLog().Print(4, _T("bufsize expanded to %d\n"), m_netbufsize);
}
...
void ClientConnection::CheckZlibBufferSize(int bufsize)
{
	...
	GetVNCLog().Print(4, _T("zlibbufsize expanded to %d\n"), m_zlibbufsize);
...
void ClientConnection::AddUpdateListener(VNCScreenUpdateListener *pListener)
{
...
void ClientConnection::RemoveUpdateListener(VNCScreenUpdateListener *pListener)
{
    omni_mutex_lock l(m_suMutex);
    ScreenUpdateListenerList::iterator i = std::find(m_suListeners.begin(), m_suListeners.end(), pListener);
    if (i != m_suListeners.end())
        m_suListeners.erase(i);
}
